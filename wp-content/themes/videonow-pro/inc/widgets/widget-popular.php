 ...
 * @since      1.0.0
 */
class videonow_pro_Popular_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-videonow_pro-popular widget_posts_thumbnail',
			'description' => __( 'Display popular posts by comments with thumbnails.', 'videonow-pro' )
		);

		// Create the widget.
		parent::__construct(
			'videonow_pro-popular',                                   // $this->id_base
			__( '&raquo; Popular Posts', 'videonow-pro' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $popular = get_transient( 'videonow_pro_popular_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'videonow_pro_popular_widget_' . $this->id, $popular );
				...
				while ( $popular->have_posts() ) : $popular->the_post();
					...
					echo '<li class="clear">';
						...
						if (!empty($video_length)) { 
							echo '<span class="video-length">' .$video_length . '</span>';
						}
						echo '</div>' . '</a>';
						echo '<div class="entry-wrap"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">' . esc_attr( get_the_title() ) . '</a>'; 

						if ( $instance['show_views'] ) :
							echo '<div class="entry-meta">' . videonow_pro_get_post_views(get_the_ID()) . '</div>';
						endif;
					echo '</div></li>';
	...
	function flush_widget_transient() {
		delete_transient( 'videonow_pro_popular_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Popular Posts', 'videonow-pro' ),
			'limit'     => 5,
			'show_views' => true
		);
	...
	?>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_views' ); ?>">
				<?php _e( 'Display post views?', 'videonow-pro' ); ?>
			</label>
		</p>
